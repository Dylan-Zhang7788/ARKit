...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.fc = nn.Linear(512 * block.expansion, self.num_arkit)
...
def _resnet(arch, block, layers, pretrained, progress, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                               progress=progress)
        loaded_state_dict = key_deletion(state_dict)
        #loaded_state_dict = key_deletion(torch.load(model_urls[arch]))
        

        model_state_dict = model.state_dict()
        pruned_dict = {k:v for (k,v) in model_state_dict.items() if k in loaded_state_dict}
        model_state_dict.update(pruned_dict)
