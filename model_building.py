...
class SynergyNet(nn.Module):
	...
	def reconstruct_vertex_62(self, param, whitening=True, dense=False, transform=True, lmk_pts=68):
		...
		if whitening:
			...
			elif param.shape[1] == 156:
				#pdb.set_trace()
				param_ = param.clone()
				param_[:, :12] = param[:, :12] * self.param_std[None, :12] + self.param_mean[None, :12]
			else:
		...
		if dense:
			...
			if param.shape[1] == 62:
				...
				vertex = p @ (self.u + self.w_shp @ alpha_shp + self.w_exp @ alpha_exp).contiguous().view(-1, 107127, 3).transpose(1, 2) + offset
			else:
				raise RuntimeError('length of params mismatch')
	...
	def forward(self, input, target):
		...
		vertex_lmk = self.reconstruct_vertex_62(_3D_attr, dense=False,lmk_pts=68)
		vertex_GT_lmk = self.reconstruct_vertex_62(_3D_attr_GT, dense=False, lmk_pts=68)
		self.loss['loss_LMK_f0'] = 0.05 *self.LMKLoss_3D(vertex_lmk, vertex_GT_lmk, kp=True)
		self.loss['loss_Param_In'] = 0.02 * self.ParamLoss(_3D_attr, _3D_attr_GT, mode='normal')
		point_residual = self.forwardDirection(vertex_lmk, avgpool, _3D_attr[:,12:92], _3D_attr[:,92:156])
		...
		self.loss['loss_Param_S2'] = 0.02 * self.ParamLoss(_3D_attr_S2, _3D_attr_GT, mode='normal')
		self.loss['loss_Param_S1S2'] = 0.001 * self.ParamLoss(_3D_attr_S2, _3D_attr, mode='normal')
...
class ARkitNet(nn.Module):
	def __init__(self, arch):
		super(ARkitNet, self).__init__()
		self.backbone = getattr(resnet_backbone, arch)(pretrained=True)
		self.reverseDirection = MLP_rev(68)
		self.loss = {'loss': 0.0}
	...
	def forward(self, input, target):
		#pdb.set_trace()
		point = self.backbone.forward_arkit(input)
		self.loss['loss'] = self.criterion(point, target).sum(1)
		return self.loss
