...
from utils.ddfa import ARkitDataset, ToTensor, Normalize, SGD_NanHandler, CenterCrop, Compose_GT, ColorJitter
from utils.ddfa import str2bool, AverageMeter
from utils.io import mkdir
from model_building import SynergyNet as SynergyNet

from model_building import ARkitNet
from benchmark_validate_v2 import benchmark_pipeline
import pdb
...
def parse_args():
    ...
    print("args.devices_id.split(',')", args.devices_id.split(','))
    args.devices_id = [int(d) for d in args.devices_id.split(',')]
    print("args.devices_id", args.devices_id)
...
def cal_nme(input, target):
    input = input.view(-1, 3)
    target = target.view(-1, 3)
    input = input.cpu().numpy()
    target = target.cpu().numpy()
    idx1, idx2 = 1101, 1069
    dn1 = max(1e-6, np.linalg.norm(target[idx1] - target[idx2]))
    nme = np.mean(np.linalg.norm(input - target, axis=1)) / dn1
    return nme
...
def valid(valid_loader, model):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(valid_loader):
            input = input.cuda()
            output = model.module.forward_test(input)
            # output = model.forward_test(inputs)

            for i in range(output.shape[0]):
                nme = cal_nme(output[i], target[i])
                nmes.append(nme)
    nme = np.mean(nmes)
    print("test nme", nme)
...
def main():
    ...
    model = ARkitNet(args.arch)
    torch.cuda.set_device(args.devices_id[0])
    if args.model_path:
        checkpoint = torch.load(args.model_path, map_location=lambda storage, loc: storage)['state_dict']
        model = nn.DataParallel(model, device_ids=args.devices_id) 
    ...
    train_dataset = ARkitDataset(
        root=os.path.join(args.root, "train_data"),
        gt_transform=True,
    ...
    valid_dataset = ARkitDataset(root=os.path.join(args.root, "valid_data"),
                            transform=transforms.Compose(
                            [ToTensor(), CenterCrop(5, mode='test'), Normalize(mean=127.5, std=130)]))
